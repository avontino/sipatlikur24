    ...
    <div class="card shadow mb-4">
      ...
      <div class="card-body p-0">
        <div class="table-responsive p-2">
          <table id="tableRekapJurnal" class="table table-bordered table-striped table-hover align-middle m-0" style="font-size: 12.5px; width: 100%;">
            <thead class="table-light">
              <tr class="text-center">
                ...
                <th style="width: 14%; cursor: pointer;" title="Klik untuk mengurutkan total mapel">Total Mapel <i class="fas fa-sort ms-1 small text-muted"></i></th>
                <th style="width: 15%; cursor: pointer;" title="Klik untuk mengurutkan mapel terisi">Mapel Terisi <i class="fas fa-sort ms-1 small text-muted"></i></th>
                <th style="width: 15%; cursor: pointer;" title="Klik untuk mengurutkan mapel kosong">Mapel Kosong <i class="fas fa-sort ms-1 small text-muted"></i></th>
                <th style="width: 11%; cursor: pointer;" title="Klik untuk mengurutkan persentase kepatuhan">% Keterisian <i class="fas fa-sort ms-1 small text-muted"></i></th>
            ...
            <tbody>
              @forelse($rekapPerKelas as $idx => $r)
                ...
                <tr>
                  ...
                  <td class="text-center fw-semibold" data-order="{{ $r['total_mapel'] }}">{{ $r['total_mapel'] }} Mapel <small class="text-muted">({{ $r['total_hari'] }} Hari)</small></td>
                  <td class="text-center" data-order="{{ $r['terisi_mapel'] }}">
                    <span class="badge bg-success px-2 py-1" style="font-size: 12px;">
                      <i class="fas fa-check-circle me-1"></i> {{ $r['terisi_mapel'] }} Terisi
                    </span>
                  </td>
                  <td class="text-center" data-order="{{ $r['kosong_mapel'] }}">
                    @if($r['kosong_mapel'] > 0)
...
@push('scripts')
<style>
  #tableRekapJurnal thead th {
    user-select: none;
    -webkit-user-select: none;
  }
  #tableRekapJurnal thead th:not(:first-child):not(:last-child) {
    cursor: pointer !important;
    transition: background-color 0.15s ease;
  }
  #tableRekapJurnal thead th:not(:first-child):not(:last-child):hover {
...
$(document).ready(function() {
  initRekapJurnalTable();
});

function initRekapJurnalTable() {
  if (typeof $.fn.DataTable !== 'undefined') {
    if ($.fn.DataTable.isDataTable('#tableRekapJurnal')) {
      $('#tableRekapJurnal').DataTable().destroy();
    }
    var table = $('#tableRekapJurnal').DataTable({
      paging: false,
      searching: true,
      info: false,
      order: [[6, 'desc']], // Default: urut dari % Keterisian tertinggi (kelas paling rajin)
      columnDefs: [
    ...
    initFallbackSorting('tableRekapJurnal');
  }
}

function initFallbackSorting(tableId) {
