...
Route::group(['middleware'=>['auth', 'force.password.change']],function(){
			...
			Route::get('/admin/sync-walikelas', function() {
				if (!auth()->user()->hasRole('admin')) {
					return redirect('/dashboard')->with('gagal', 'Akses ditolak.');
				}
				$walis = [
					'7A' => ['Dwi Rahmawati'],
					'7B' => ['Elsye', 'Sandra'],
				...
				foreach ($walis as $kelas => $keywords) {
					...
					if ($user) {
						...
						$updated[] = "{$kelas}: {$user->name}";
					}
				}
				return redirect('/dashboard')->with('sukses', 'Berhasil memperbarui data Wali Kelas (' . count($updated) . ' kelas): ' . implode(', ', $updated));
			});
