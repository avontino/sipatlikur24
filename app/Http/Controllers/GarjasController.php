...
class GarjasController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        } else if (Auth::user()->hasRole('pembina') || Auth::user()->hasRole('admin')) {
            $rules = array_merge($rules, [
        ...
        try {
            ...
            if (Auth::user()->hasRole('siswa')) {
                ...
                $bulanName = $monthNames[$request->bulan] ?? $request->bulan;
                
                $title = "Pengisian Garjas Baru";
                $message = "Siswa " . Auth::user()->name . " (Kelas " . $siswa->kelas . ") telah mengisi/memperbarui data latihan Garjas untuk Bulan " . $bulanName . " " . $request->tahun . ". Mohon berikan penilaian.";
                
                $pembinaAndKesiswaan = \App\Models\User::whereIn('role', ['pembina', 'kesiswaan'])->get();
                foreach ($pembinaAndKesiswaan as $u) {
                    $u->sendNotification($title, $message, '/garjas', 'garjas');
                }
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        // Cek authorization
        if (Auth::user()->hasRole('siswa') && $garjas->nis != Auth::user()->username) {
            if ($request->ajax()) {
                return response()->json([
                    'success' => false,
